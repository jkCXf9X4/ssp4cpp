...
{
    using namespace std;
    using AnalysisNode = sim::analysis_graph::AnalysisNode;

    inline auto log = common::Logger("sim::analysis_graph", common::LogLevel::ext_trace);

    map<string, Model *> get_new_models(ssp4cpp::Ssp &ssp)
    {
        log.ext_trace("[{}] init", __func__);
        map<string, Model *> models;
        for (auto &[ssp_resource_name, local_resource_name] : ssp::ext::get_resources(ssp))
        {
            auto m = new Model(ssp_resource_name, local_resource_name);
    ...
    map<string, Connector *> get_new_connectors(ssp4cpp::Ssp &ssp)
    {
        ...
        if (ssp.ssd.System.Elements.has_value())
        {
            ...
            {
                auto c = new Connector(component->name.value(), connector->name);
    ...
    AnalysisGraph create_analysis_graph(ssp4cpp::Ssp &ssp)
    {
        log.ext_trace("[{}] init", __func__);
        ...
        for (auto &[name, c] : connections)
        {
            ...
            auto source_connector = connectors[source_connector_name];
            auto target_connector = connectors[target_connector_name];
